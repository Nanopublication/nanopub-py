...
def test_find_by_ref_calls_search(mock_client, fdo_query):
    ...
    mock_client._search.assert_called_once_with(
        "RAQiQjx3OiO9ra9ImWl9kpuDpT8d3EiBSrftckOAAwGKc/find-fdos-by-ref",
        {"refid": "abc123"},
    )
...
def test_get_feed_calls_search(mock_client, fdo_query):
    ...
    mock_client._search.assert_called_once_with(
        "RAP1G35VvTs3gfMaucv_xZUMZuvjB9lxM8tWUGttr5mmo/get-fdo-feed",
        {"creator": "https://orcid.org/1234"},
    )
...
def test_get_favorite_things_calls_search(mock_client, fdo_query):
    ...
    mock_client._search.assert_called_once_with(
        "RAsyc6zFFnE8mblnDfdCCNRsrcN1CSCBDW9I4Ppidgk9g/get-favorite-things",
        {"creator": "https://orcid.org/5678"},
    )
