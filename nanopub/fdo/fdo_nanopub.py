...
class FdoNanopub(Nanopub):
    ...
    def __init__(
        self,
        fdo_id: rdflib.URIRef | str,
        label: str,
        fdo_profile: str = None,
        *args,
        **kwargs,
    ):
    ...
    def _init_core_fdo_triples(self, label: str):
        ...
        self.pubinfo.add(
            (
                self.metadata.np_uri,
                RDFS.label,
                rdflib.Literal(f"FAIR Digital Object: {label}"),
            )
        )
    ...
    def handle_to_nanopub(cls, handle: str, **kwargs) -> "FdoNanopub":
        ...
        np = cls(
            fdo_id=handle, label=label or handle, fdo_profile=fdo_profile, **kwargs
        )
    ...
    def add_fdo_profile(self, profile_uri: rdflib.URIRef | str):
        ...
        self.assertion.add((self.fdo_uri, DCTERMS.conformsTo, profile_uri))
        self.pubinfo.add(
            (HDL[FDO_PROFILE_HANDLE], RDFS.label, rdflib.Literal("FdoProfile"))
        )
    ...
    def add_fdo_data_ref(self, data_ref: rdflib.Literal | str):
        ...
        self.pubinfo.add(
            (HDL[FDO_DATA_REF_HANDLE], RDFS.label, rdflib.Literal("DataRef"))
        )

    def add_attribute(
        self, attr_handle: rdflib.URIRef | str, value: rdflib.Literal | str
    ):
