...
class Nanopub:
    ...
    def __init__(
        ...
        if not rdf and not source_uri:
            self._head.add((self._metadata.namespace[""], RDF.type, NP.Nanopublication))
            self._head.add(
                (
        ...
        self._handle_generated_at_time(
            self._conf.add_pubinfo_generated_time, self._conf.add_prov_generated_time
        )
        assertion_attributed_to = self._conf.assertion_attributed_to
    ...
    def sign(self) -> None:
        ...
        if self._metadata.signature:
            raise MalformedNanopubError(
                f"The nanopub have already been signed: {self.source_uri}"
            )

        if self.is_valid:
            self._replace_blank_nodes(self._rdf)
            signed_g = add_signature(
                self.rdf, self._conf.profile, self._metadata.namespace, self._pubinfo
            )
    ...
    def get_source_uri_from_graph(self) -> Optional[str]:
        ...
        for s in self._rdf.subjects(rdflib.RDF.type, NP.Nanopublication):
            extract_trusty = re.search(
                r"^[a-z0-9+.-]+:\/\/[a-zA-Z0-9\/._-]+\/(RA.*)$", str(s), re.IGNORECASE
            )
            if extract_trusty:
                return str(s)
        return None
    ...
    def _handle_assertion_attributed_to(
        self, assertion_attributed_to: Optional[str]
    ) -> None:
        """Handler for `Nanopub` constructor."""
    ...
    def _validate_nanopub_arguments(
        ...
        if self._provenance:
            if derived_from and (None, PROV.wasDerivedFrom, None) in self._provenance:
                raise MalformedNanopubError(
        ...
        if self._pubinfo:
            if introduces_concept and (None, NPX.introduces, None) in self._pubinfo:
                raise MalformedNanopubError(
