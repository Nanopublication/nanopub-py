        ...
        if looks_like_handle(profile_uri):
            ...
            jsonschema_entry = next(
                (
                    ...
                    if v.get("type") == "21.T11966/JsonSchema"
                ),
                None,
            )
            if not jsonschema_entry:
                return ValidationResult(
                    False, ["JSON Schema entry not found in FDO profile."], []
                )
            ...
            if profile:
                ...
                try:
                    profile_response = requests.get(
                        profile_uri, headers={"Accept": "application/ld+json"}
                    )
                    ...
                    if not shape_uri:
                        return ValidationResult(
                            False, ["No hasShape found in profile JSON-LD"], []
                        )
                    ...
                    try:
                        profile_api_url = _profile_landing_page_uri_to_api_url(
                            str(profile_uri)
                        )
                        profile_response = requests.get(profile_api_url)
                        profile_data = profile_response.json()
                        ...
                        if not jsonschema_url:
                            return ValidationResult(
                                False, ["JSON Schema $ref not found."], []
                            )
                        ...
                        return ValidationResult(
                            False, [f"Validation fallback error: {str(e2)}"], []
                        )

        if shape_graph is None:
            return ValidationResult(
                False, ["SHACL shape graph could not be created."], []
            )
